...
def affine_relu_backward(dout, cache):
    ...
    """
    fc_cache, batch_cache,relu_cache,_ = cache
    da = relu_backward(dout, relu_cache)
    # dc,_,_= batchnorm_backward(da,batch_cache)
    dx, dw, db = affine_backward(da, fc_cache)
    return dx, dw, db

def affine_relu_backward_forconv(dout, cache):
    """Backward pass for the affine-relu convenience layer.
    """
...
def affine_layernorm_relu_backward(dout, cache):
    """Backward pass for the affine-relu convenience layer.
    """
    fc_cache, layer_cache,relu_cache,_ = cache
    da = relu_backward(dout, relu_cache)
    dc,dgamma,dbeta= layernorm_backward(da,layer_cache)
    dx, dw, db = affine_backward(dc, fc_cache)
    return dx, dw, db,dgamma,dbeta

def affine_batchnorm_relu_dropout_backward(dout, cache):
    fc_cache, layer_cache,relu_cache,dropout_cache = cache
    dd=dropout_backward(dout,dropout_cache)
    da = relu_backward(dd, relu_cache)
    dc,dgamma,dbeta= batchnorm_backward(da,layer_cache)
    dx, dw, db = affine_backward(dc, fc_cache)
    return dx, dw, db,dgamma,dbeta
