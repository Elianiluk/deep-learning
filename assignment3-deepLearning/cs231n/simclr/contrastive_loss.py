...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        #first one
        mone=torch.exp(sim(z_k,z_k_N)/tau)
        mechane=0
        for i in range(out.shape[0]):
          if i==k:
            pass
          else:
            mechane+=torch.exp(sim(z_k,out[i])/tau)
        total1=-torch.log(mone/mechane)

        #second one
        mone1=torch.exp(sim(z_k_N,z_k)/tau)
        mechane1=0
        for j in range(out.shape[0]):
          if j==k+N:
            pass
          else:
            mechane1+=torch.exp(sim(z_k_N,out[j])/tau)
        total2=-torch.log(mone1/mechane1)

        total_loss+=total1+total2
...
def sim_positive_pairs(out_left, out_right):
    ...
    ans=torch.sum(out_left*out_right,dim=1,keepdim=True)
    i_norm=torch.linalg.norm(out_left,dim=1,keepdim=True)
    j_norm=torch.linalg.norm(out_right,dim=1,keepdim=True)
    ans2=(i_norm*j_norm)
    pos_pairs= ans/(ans2)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    simi=sim_positive_pairs(out_left,out_right)
    ...
    loss = None
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    # positive_indices = torch.arange(N, device=device)
    # denom_pos = denom[positive_indices]
    denom=denom.view(N,-1)
