...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        key= key.view(N, T, H, self.head_dim)
        value= value.view(N, T, H, self.head_dim)
        query=query.view(N,S,H,self.head_dim)
        ...
        scale=math.sqrt(E/H)
        now=(torch.matmul(query,key))/scale

        if(attn_mask!=None):
          # attn_mask=attn_mask.view(1,1,*attn_mask.size())
          now=now.masked_fill(attn_mask<0.5,float('-inf'))

        soft=torch.nn.functional.softmax(now,dim=-1)
        drop=self.attn_drop(soft)

        output=torch.matmul(drop,value)
